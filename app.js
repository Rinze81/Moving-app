...
function renderSummary() {
  ...
  const movingPlanSummary = document.getElementById("movingPlanSummary");
  movingPlanSummary.textContent = state.settings.movingDate
    ? `${dateLabel(state.settings.movingDate)} を基準に予定を確認中。目的地は ${state.settings.targetStation || "未設定"} です。`
    : "引っ越し日を入れると、スケジュールの見通しが出しやすくなります。";
...
function registerEvents() {
  ...
  const generateSuggestedTasksButton = document.getElementById("generateSuggestedTasks");
  if (generateSuggestedTasksButton) {
    generateSuggestedTasksButton.addEventListener("click", generateSuggestedTasks);
  }
